...
public final class PetHeadClientHandler {
    private static final java.util.Map<UUID, Long> CLIENT_PET_HEAD_UNTIL =
    ...
    public static UUID resolveTargetMaidUuid(Minecraft mc) {
        ...
        UUID childInteractionMaidUuid =
                ChildInteractionClientState.getLocalInteractionMaidUuid();

        if (childInteractionMaidUuid != null) {
            return childInteractionMaidUuid;
        }

        UUID interactionMaidUuid =
                HugClientState.getLocalInteractionMaidUuid();
    ...
    public static void triggerInteraction(Minecraft mc) {
        ModNetworking.sendHugMaid(
                new HugMaidPayload(resolveTargetMaidUuid(mc))
        );
    }
    ...
    public static void triggerUnifiedInteraction(Minecraft mc) {
        ...
        UUID childMaidUuid =
                resolveDirectChildInteractionTargetMaidUuid(mc);
    ...
    public static void triggerHugPoseToggle(Minecraft mc) {
        ModNetworking.sendToggleHugPose(
                new ToggleHugPosePayload(resolveTargetMaidUuid(mc))
        );
    }
    ...
    public static void triggerSeatedPetHeadShortcut(Minecraft mc) {
        ...
        UUID liftedMaidUuid =
                MaidLiftManager.getLocalLiftedMaidUuid(mc.player);
        ...
        UUID carriedChildUuid =
                MaidCarryChildManager.getLocalCarriedChildUuid(mc.player);
    ...
    public static void triggerKiss(Minecraft mc) {
        ModNetworking.sendKissMaid(
                new KissMaidPayload(resolveTargetMaidUuid(mc))
        );
    }

    public static void triggerDialogueChoiceResult(
            Minecraft mc,
        ...
        UUID resolvedTarget =
                maidUuid != null
                        ? maidUuid
                        : resolveTargetMaidUuid(mc);

        ModNetworking.sendDialogueChoiceResult(
                new DialogueChoiceResultPayload(
                        resolvedTarget,
                        positiveFavor,
                        neutralFavor,
                        negativeFavor,
     ...
     * 小女仆互动面板的唯一客户端入口。
     */
    public static void triggerChildInteraction(
            Minecraft mc,
            @Nullable UUID maidUuid
    ) {
        if (ChildInteractionClientState.isLocalPlayerInteracting()) {
            ...
            if (mc != null
                    && mc.screen instanceof HugActionScreen screen
                    && screen.isChildInteractionScreen()) {

                mc.setScreen(null);
            }
        }

        ModNetworking.sendChildInteraction(
                new ChildInteractionPayload(maidUuid)
        );
    }

    public static void triggerPetHead(
            Minecraft mc,
            @Nullable UUID maidUuid
    ) {
        ModNetworking.sendPetHead(
                new PetHeadPayload(maidUuid)
        );

        markClientPredictedPetHead(mc, maidUuid);
    }

    public static void triggerLift(
            Minecraft mc,
            @Nullable UUID maidUuid
    ) {
        ModNetworking.sendLiftMaid(
                new LiftMaidPayload(maidUuid)
        );
    }

    public static void triggerCarryChild(
            Minecraft mc,
            @Nullable UUID maidUuid
    ) {
        ModNetworking.sendCarryChildMaid(
                new CarryChildMaidPayload(maidUuid)
        );
    }

    @Nullable
    private static EntityMaid resolveTargetMaid(
            Minecraft mc,
            @Nullable UUID maidUuid
    ) {
    ...
    private static void markClientPredictedPetHead(
            Minecraft mc,
            @Nullable UUID maidUuid
    ) {
        ...
        boolean hugVariant =
                HugClientState.isLocalPlayerHugging()
                        && maidUuid != null
                        && maidUuid.equals(
                        HugClientState.getLocalInteractionMaidUuid()
                );
        ...
        long endTick =
                mc.level.getGameTime()
                        + PetHeadManager.clientPredictedAnimTicks(hugVariant);

        CLIENT_PET_HEAD_UNTIL.put(
                maid.getUUID(),
                endTick
        );

        PetHeadManager.startClientPredictedPetHead(
                maid,
                hugVariant
        );
    }
    ...
    private static void cleanupClientPredictedPetHead(Minecraft mc) {
        ...
        while (iterator.hasNext()) {
            ...
            EntityMaid maid =
                    resolveTargetMaid(mc, entry.getKey());
     ...
     */
    @Nullable
    private static UUID resolveDirectChildInteractionTargetMaidUuid(
            Minecraft mc
    ) {
        if (mc == null) {
            return null;
