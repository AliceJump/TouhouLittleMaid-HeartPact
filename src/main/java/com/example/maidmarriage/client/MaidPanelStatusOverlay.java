...
public final class MaidPanelStatusOverlay {
    ...
    public static void onRenderMaidPanel(ScreenEvent.Render.Post event) {
        ...
        Component marriageText =
                (marriage != null && marriage.married())
        ...
        if (MaidChildEntity.shouldStayChild(maid)) {
            Component stageText = Component.translatable(
                    "panel.maidmarriage.child_stage."
                            + MaidChildEntity.resolveGrowthStage(maid)
                            .name()
                            .toLowerCase()
            );

            physiologyText = Component.translatable(
                    "panel.maidmarriage.physiology.child_stage",
                    stageText
            );
        } else {
            physiologyText =
                    (pregnancy != null && pregnancy.firstExperience())
                            ? (pregnancy.pregnant()
        ...
        MaidMoodData mood =
                maid.getOrCreateData(
                        ModTaskData.MOOD_DATA,
                        MaidMoodData.EMPTY
                );
