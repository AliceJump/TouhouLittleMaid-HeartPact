...
public final class HugCameraZoom {
    ...
    static {
        HugCameraSettingsStore.Settings settings =
                HugCameraSettingsStore.loadOrDefault(defaultSettings());
    ...
    public static void playTransientZoom(double targetScale, long durationTicks) {
        ...
        transientZoomUntilClientTick = Math.max(
                transientZoomUntilClientTick,
                clientTick + Math.max(1L, durationTicks)
        );
    }
    ...
    public static void setCameraPitchOffsetDegrees(float value) {
        cameraPitchOffsetDegrees =
                (float) clamp(value, MIN_CAMERA_PITCH_OFFSET, MAX_CAMERA_PITCH_OFFSET);
    }
    ...
    public static boolean savePersistentSettings() {
        return HugCameraSettingsStore.save(
                new HugCameraSettingsStore.Settings(
                        hugFovScale,
                        cameraPitchOffsetDegrees
                )
        );
    ...
    public static void onComputeFov(ViewportEvent.ComputeFov event) {
        ...
        boolean adultInteractionZoomActive =
                HugClientState.isLocalPlayerInteracting();

        boolean childInteractionZoomActive =
                ChildInteractionClientState.isLocalPlayerInteracting();

        if (!adultInteractionZoomActive
                && !childInteractionZoomActive
                && clientTick > transientZoomUntilClientTick) {
            return;
        }

        double scale =
                adultInteractionZoomActive || childInteractionZoomActive
                        ? hugFovScale
                        : 1.0D;
    ...
    private static HugCameraSettingsStore.Settings defaultSettings() {
        return new HugCameraSettingsStore.Settings(
                DEFAULT_HUG_FOV_SCALE,
                DEFAULT_CAMERA_PITCH_OFFSET
        );
    }
